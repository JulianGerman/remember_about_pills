...
class ModalBottomSheet extends StatefulWidget {
  const ModalBottomSheet({@required this.title, this.onSubmit});
                      ...
                      child: GridView.builder(
                          ...
                          itemBuilder: (ctx, index) {
                            return TextButton(
                                ...
                                // child: Text('Set time'),
                                // child: _timeOfDayPills[state] == null
                                child: Text('Set time'.toUpperCase()),
                                // : Text(_timeOfDayPills[index]),
                                style: ButtonStyle(
  ...
  _selectTime(BuildContext context, int index) async {
    final TimeOfDay timeofPill = await showTimePicker(
        ...
        initialTime: TimeOfDay(
            hour: int.parse(getHour(index).split(":")[0]),
            minute: int.parse(getHour(index).split(":")[1])));
    if (timeofPill != null) {
      setState(() {
